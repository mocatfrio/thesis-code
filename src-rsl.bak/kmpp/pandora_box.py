  ...
  def update(self, dsl):
    for p in dsl:
      if 'last_ts' in dsl[p] and 'last_prob' in dsl[p]:
        ...
        # logging.info('\t\t Update score pbox[p{}][t{}] + {} = {}'.format(p, dsl[p]['ts'], dsl[p]['prob'], self.box[p][dsl[p]['ts']]))

  def update_score(self, p, ts, last_ts, prob, last_prob):
    for i in range(last_ts + 1, ts):
      self.box[p][i] += last_prob
      # logging.info('\t\t Update score pbox[p{}][t{}] + {} = {}'.format(p, i, last_prob, self.box[p][i]))
    self.box[p][ts] += prob
    # logging.info('\t\t Update score pbox[p{}][t{}] + {} = {}'.format(p, ts, prob, self.box[p][ts]))
