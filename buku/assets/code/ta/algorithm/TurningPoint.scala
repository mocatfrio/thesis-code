...
object TurningPoint {
  def processLandmark(grid: Grid, nodeS: Node, edge: Edge, nodeE: Node): Unit = {
    ...
    Se.foreach { obj =>
      ...
      if (llDistance >= 0 & llDistance <= edge.length) {
        val landmarkLeft = 
            new LandmarkLeft(obj.distance - D_EPSILON, Some(edge.id), obj.id)
      ...
      if (lrDistance <= edge.length & lrDistance >= 0) {
        val landmarkRight =
            new LandmarkRight(obj.distance + D_EPSILON, Some(edge.id), obj.id)
    ...
    spNodeS
      ...
      .foreach { obj =>
        ...
        if (distanceLandmark >= 0 & distanceLandmark <= edge.length) {
          val landmarkRight = 
              new LandmarkRight(distanceLandmark, Some(edge.id), obj.id)
          Q.enqueue(landmarkRight)
        }
      }
    ...
    spNodeE
      ...
      .foreach { obj =>
        ...
        if (distanceLandmark >= 0 & distanceLandmark <= edge.length) {
          val landmarkLeft =
              new LandmarkLeft(distanceLandmark, Some(edge.id), obj.id)
    ...
    SP.foreach { o =>
      ...
      dominatedObjects.foreach { dominatedObj =>
        ...
        if (distanceO > distanceDominatedObj) {
          ...
          if (distanceLandmark >= 0 & distanceLandmark <= edge.length) {
            val landmark = 
                new LandmarkLeftMid(distanceLandmark, Some(edge.id), o.id, dominatedObj.id)
          ...
          if (distanceLandmark >= 0 & distanceLandmark <= edge.length) {
            val landmark = 
                new LandmarkRightMid(distanceLandmark, Some(edge.id), o.id, dominatedObj.id)
  ...
  def findDistance(obj: Object, edge: Edge, spNodeS: Set[Object], 
        spNodeE: Set[Object]): Double = {
    if (obj.edgeId == edge.id) {
      edge.length * obj.position
    } else {
  ...
  def processLandmark(landmarks: List[Landmark], spNodeS: Set[Object], 
        spNodeE: Set[Object], edge: Edge): Unit = {
