...
object TheAlgorithm {
  def TheAlgorithm(grid: Grid, stream: Stream): Grid = {
    var Q: mutable.Queue[NodeQueue] = mutable.Queue[NodeQueue]()
  ...
  def computeTurningPoint(grid: Grid, edges: mutable.Map[Int, Edge],
    nodes: mutable.Map[Int, Node], updatedNodes: Set[Int]): Unit = {
      edges.values
  ...
  def insertToNode(grid: Grid, node: Node,
    ...
    val finalObjects = updatedObjects +
        Object(rawObject.id, rawObject.edgeId, skyProbU, isImpossible = false, 
          node.id, rect, distance, rawObject.position)
  ...
  def getDominationProbability(tree: RTree[Point2d], ddrRect: Rect2d, objectId: Int)
    : Double = {
  ...
  def findPDROverlappedObjects(node: Node, rect: Rect2d): Set[Object] = {
    ...
    val objectIds = overlappedPoints.toList
                      .filter(_.isInstanceOf[Point2d]).map(_.o).toSet
  ...
  def findPDDOverlappedObjects(node: Node, rect: Rect2d, currentId: Int)
    : Set[Object] = {
    val tree = node.tree
