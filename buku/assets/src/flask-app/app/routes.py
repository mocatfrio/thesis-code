...
def get_data(session_name):
  data = json.load(open(app.config['SESSION_FILE']))
  product_path = app.config['UPLOAD_DIR'] + '/' + 
  		data['session'][session_name]['product_data']
  customer_path = app.config['UPLOAD_DIR'] + '/' + 
  		data['session'][session_name]['customer_data']
...
def search_session():
  if request.method == 'GET':
    return render_template('search_session.html', title = 'Search', 
    		session = read_session())
  else:
    session_name = request.form.get('session')
    return redirect ( url_for('search', session_name = session_name) )
...
def search(session_name):
  if request.method == 'GET':
    ...
    pandora_path = app.config['SESSION_DIR'] + '/' + session_name + 
    		'/pandora_box.csv'

    result = ss.solution(pandora_path, k_product, time_start, time_end)
    result_json = json.dumps(result)
    loaded_r = json.loads(result_json)
    return render_template('search.html', title = 'Search', result = loaded_r, 
    	param = [k_product, time_start, time_end], session_name = session_name) 
...
def vis(session_name):
  ...
  return render_template('vis.html', title = 'Visualization', 
  	session_name = session_name, metadata = metadata, 
  	product = p_data, customer = c_data)
