...
class DynamicSkyline:
  ...
  def product_out(self, cust_id, ts, prod_id):
    ...
    try:
      if 'dominating' in self.customer[cust_id]['dsl'][prod_id]:
        active_child = self.find_active_child(
                        self.customer[cust_id]['dsl'][prod_id])
      del self.customer[cust_id]['dsl'][prod_id]
      if active_child:
        self.customer[cust_id]['dsl'] = self.update_dsl(active_child, 
                                                        cust_id, cust_value)
  ...
  def product_in(self, cust_id, ts, prod_id):
    cust_value = self.customer[cust_id]['value']
    self.customer[cust_id]['dsl'] = self.update_dsl(prod_id, cust_id, 
                                                    cust_value)
    self.update(cust_id, ts)
          ...
          if dsl_id[index] is not None and dsl_id[i] is not None:
            if i < index:
              if self.is_dominating(dsl[dsl_id[i]], dsl[dsl_id[index]]):
                dsl, dsl_id[index] = self.update_cand(dsl, dsl_id[i], 
                                                      dsl_id[index])
                break
            elif i > index:
  ...
  def update(self, cust_id, ts):
    for prod_id in self.customer[cust_id]['dsl']:
      try:
        ...
        self.customer[cust_id]['dsl'][prod_id]['prob'] = self.calc_probability(
                                                  self.customer[cust_id]['dsl'])
      except KeyError:
        pass

  def update_history(self, cust_id):
    for prod_id in self.customer[cust_id]['dsl']:
      self.customer[cust_id]['dsl'][prod_id]['last_ts'] = 
      self.customer[cust_id]['dsl'][prod_id]['ts']
      self.customer[cust_id]['dsl'][prod_id]['last_prob'] = 
      self.customer[cust_id]['dsl'][prod_id]['prob']
